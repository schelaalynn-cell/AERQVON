...
export function QRCode({ value, size = 220 }: QRCodeProps) {
  ...
  return (
    <canvas ref={canvasRef} width={size} height={size} className="rounded-2xl" style={{ width: size, height: size }} />
  );
}
...
function generateQRMatrix(text: string): number[][] {
  ...
  return createMatrix(version, data);
}

function selectVersion(byteLength: number): number {
  const capacities = [17,32,53,78,106,134,154,192,230,271,321,367,425,458,520,586,644,718,792,858,929,1003,1091,1171,1273,1367,1465,1528,1628,1732,1840,1952,2068,2188,2303,2431,2563,2699,2839,2983];
  for (let v = 0; v < capacities.length; v++) { if (capacities[v] >= byteLength + 2) return v + 1; }
  return 40;
}
...
function createMatrix(version: number, data: Uint8Array): number[][] {
  ...
  if (version >= 2) { for (const [r, c] of getAlignmentPositions(version)) placeAlignment(matrix, reserved, r, c); }
...
function placeFinder(matrix: number[][], reserved: boolean[][], r: number, c: number) {
  for (let dr = -1; dr <= 7; dr++) {
    for (let dc = -1; dc <= 7; dc++) {
      ...
      matrix[rr][cc] = val; reserved[rr][cc] = true;
    }
  }
}
...
function placeAlignment(matrix: number[][], reserved: boolean[][], r: number, c: number) {
  for (let dr = -2; dr <= 2; dr++) {
    for (let dc = -2; dc <= 2; dc++) {
      ...
      matrix[r + dr][c + dc] = val; reserved[r + dr][c + dc] = true;
    }
  }
}
...
function getAlignmentPositions(version: number): [number, number][] {
  ...
  for (let i = 1; i < intervals - 1; i++) positions.push(last - (intervals - 1 - i) * step);
  positions.push(last);
...
function reserveFormatInfo(matrix: number[][], reserved: boolean[][]) {
  ...
  for (let i = 0; i <= 8; i++) { if (i !== 6) { reserved[8][i] = true; matrix[8][i] = 0; reserved[i][8] = true; matrix[i][8] = 0; } }
  for (let i = 0; i < 8; i++) { const r = size - 1 - i; reserved[r][8] = true; matrix[r][8] = 0; reserved[8][size - 1 - i] = true; matrix[8][size - 1 - i] = 0; }
...
function encodeData(version: number, data: Uint8Array): number[] {
  ...
  let pi = 0;
  while (bits.length < totalDataBits) { appendBits(bits, padBytes[pi % 2], 8); pi++; }
  ...
  for (let i = 0; i < maxEc; i++) for (const block of ecBlocks) if (i < block.length) result.push(block[i]);
  return result;
}

function appendBits(arr: number[], value: number, count: number) { for (let i = count - 1; i >= 0; i--) arr.push((value >> i) & 1); }

const EC_CODEWORDS_TABLE: Record<number, number> = { 1:7,2:10,3:15,4:20,5:26,6:36,7:40,8:48,9:60,10:72,11:80,12:96,13:104,14:120,15:132,16:144,17:168,18:180,19:216,20:240,21:260,22:288,23:320,24:360,25:408,26:448,27:480,28:532,29:560,30:632,31:672,32:704,33:776,34:864,35:896,36:992,37:1056,38:1120,39:1208,40:1296 };
const TOTAL_CODEWORDS_TABLE: Record<number, number> = { 1:26,2:44,3:70,4:100,5:134,6:172,7:196,8:242,9:292,10:346,11:404,12:466,13:532,14:581,15:655,16:733,17:815,18:901,19:991,20:1085,21:1156,22:1258,23:1364,24:1474,25:1610,26:1726,27:1902,28:2031,29:2158,30:2335,31:2461,32:2605,33:2747,34:2929,35:3057,36:3253,37:3417,38:3597,39:3791,40:3993 };

function getEcCodewords(version: number): number { return EC_CODEWORDS_TABLE[version] ?? 7; }
function getTotalCodewords(version: number): number { return TOTAL_CODEWORDS_TABLE[version] ?? 26; }
...
function generateEc(data: number[], ecLength: number): number[] {
  ...
  for (const byte of data) {
    ...
    for (let i = 0; i < ecLength; i++) result[i] ^= multiplyGf(factor, genPoly[i]);
  }
  return result;
}
...
function multiplyGf(a: number, b: number): number { if (a === 0 || b === 0) return 0; return GF_EXP[GF_LOG[a] + GF_LOG[b]]; }
function getGeneratorPolynomial(degree: number): number[] {
  let poly = [1];
  for (let i = 0; i < degree; i++) { const newPoly = new Array(poly.length + 1).fill(0); for (let j = 0; j < poly.length; j++) { newPoly[j] ^= poly[j]; newPoly[j + 1] ^= multiplyGf(poly[j], GF_EXP[i]); } poly = newPoly; }
  return poly;
}

function placeData(matrix: number[][], reserved: boolean[][], codewords: number[]) {
  const size = matrix.length;
  let bitIndex = 0, direction = -1, col = size - 1;
  while (col > 0) {
    if (col === 6) col--;
    for (let i = 0; i < size; i++) {
      ...
      for (let j = 0; j < 2; j++) {
        const c = col - j;
        if (!reserved[r][c]) { const byteIdx = Math.floor(bitIndex / 8); const bitIdx = 7 - (bitIndex % 8); const bit = byteIdx < codewords.length ? (codewords[byteIdx] >> bitIdx) & 1 : 0; matrix[r][c] = bit; bitIndex++; }
      }
    }
    col -= 2; direction = -direction;
  }
}
...
function applyFormatInfo(matrix: number[][], ecLevel: number) {
  ...
  for (let i = 0; i < 7; i++) matrix[8][size - 7 + i] = (bits >> (i + 8) & 1);
}

function computeFormatBits(format: number): number {
  let result = format << 10;
  for (let i = 14; i >= 10; i--) { if ((result >> i) & 1) result ^= 0b10100110111 << (i - 10); }
